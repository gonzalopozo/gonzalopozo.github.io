		...
		<>
			...
			<span
				className={cn(
					...
					isOnline
						? hasArtwork
							? 'border-accent/30 shadow-accent/20 bg-black/50 text-white/95 shadow-lg'
							: 'border-primary/30 bg-primary/10 text-foreground shadow-primary/15 shadow-lg'
						: hasArtwork
...
function NowPlayingAtmosphere({ onArtwork }: { onArtwork: boolean }) {
	return (
		<div className="pointer-events-none absolute inset-0 overflow-hidden" aria-hidden="true">
			<span
				className={cn(
					'motion-safe:animate-now-playing-glow absolute -top-12 -right-10 size-32 rounded-full blur-2xl motion-reduce:opacity-40',
					onArtwork ? 'bg-accent/45' : 'bg-primary/25',
				)}
			/>
			<span
				className={cn(
					'motion-safe:animate-now-playing-sweep absolute top-8 -left-20 h-14 w-[150%] rotate-[-14deg] motion-reduce:hidden',
...
function NowPlayingEqualiser({ onArtwork }: { onArtwork: boolean }) {
	...
	return (
		<span
			...
			aria-hidden="true"
		>
			<span className="motion-safe:animate-now-playing-glow absolute -inset-1 rounded-full bg-current opacity-20 blur-sm motion-reduce:opacity-20" />
