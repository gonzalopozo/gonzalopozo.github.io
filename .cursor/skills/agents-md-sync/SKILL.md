...
progressive_disclosure:
  entry_point:
    summary: "Detect drift between AGENTS documentation and the codebase, then apply minimal surgical updates."
    when_to_use: "After adding/removing entities, changing dependencies, restructuring routes, adding/removing skills, or modifying project conventions."
    quick_start: "1. Gather a structural snapshot of the codebase. 2. Compare each AGENTS section against reality. 3. Apply only the edits needed. 4. Verify cross-references."
---
# AGENTS.md Sync
...
- After adding or removing a `.cursor/skills/` skill
...
| `.cursor/skills/` directories | `Glob(".cursor/skills/*/SKILL.md")` | Skill inventory |
...
2. Collect every `.cursor/skills/*/SKILL.md` path mentioned.
...
2. **Skill reference validity** — Collect every `.cursor/skills/*/SKILL.md` path mentioned across ALL AGENTS files. For each, `Glob` to confirm the SKILL.md exists. Flag any broken references.
