...
import { findSpotifyTrackForLastFmTrack, resetSpotifyTokenCacheForTesting } from '@/lib/spotify';
	...
	it('reuses a valid client credentials token between searches', async () => {
		...
		expect(
			fetcher.mock.calls.filter(
				([input]) => String(input) === 'https://accounts.spotify.com/api/token',
			),
		).toHaveLength(1);
	});
		...
		const fetcher = vi.fn(async (input: string | URL) => {
			...
			if (url === 'https://accounts.spotify.com/api/token') {
				return tokenResponse(
					fetcher.mock.calls.length === 1 ? 'first-token' : 'second-token',
					120,
				);
			}
